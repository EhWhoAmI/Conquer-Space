...
import ConquerSpace.game.universe.civilization.controllers.LimitedStar;
...
public class SystemInternalsDrawer {
    ...
    public SystemInternalsDrawer(LimitedStarSystem sys, LimitedUniverse u, Dimension bounds) {
        universe = u;
        this.bounds = bounds;
        system = sys;
        ...
        LimitedStar star = sys.getStar(0);
        if (star != null) {
            int starSize = star.getStarSize();
            int type = star.getType();
            Color c;
            switch (type) {
                case 0:
        ...
        for (int n = 0; n < sys.getPlanetCount(); n++) {
            LimitedPlanet p = sys.getPlanet(n);
    ...
    void refresh() {
        ...
        // As of version indev, there is only one star.
        //Star will be in the center
        LimitedStar star = system.getStar(0);
        if (star != null) {
            int starSize = star.getStarSize();
            int type = star.getType();
            Color c;
        ...
        for (int n = 0; n < system.getPlanetCount(); n++) {
            LimitedPlanet p = system.getPlanet(n);
            int type = p.getPlanetType();
            Color cl;
            switch (type) {
                case 0:
