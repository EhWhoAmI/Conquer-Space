...
public class WriterBMP {
     ...
     * @throws ConquerSpace.common.game.jLibNoise.noise.ExceptionInvalidParam See the preconditions.
...
//        OutputStreamWriter os = new OutputStreamWriter(new BufferedOutputStream(new FileOutputStream(new File("d:/java.txt"))));
        ...
        for (int y = height - 1; y >= 0; y--) {
            for (int x = 0; x < width; x++) {
                int pos = x + (y * width);
                ConquerSpace.common.game.jLibNoise.noise.utils.Color clr = sourceImage.image[pos];
