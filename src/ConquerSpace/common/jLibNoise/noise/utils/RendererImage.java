 ...
 * <p>
 * An application can configure the output of the image in three ways: - Specify the color gradient.
 * - Specify the light source parameters. - Specify the background image.
 * <p>
 * <b>Specify the color gradient</b>
 * <p>
 * This class uses a color gradient to calculate the color for each pixel in the destination image
 * according to the value from the corresponding position in the noise map.
 * <p>
 * A color gradient is a list of gradually-changing colors. A color gradient is defined by a list of
 * <i>gradient points</i>. Each gradient point has a position and a color. In a color gradient, the
 ...
 * <p>
 * - -1.0 maps to dark blue. - -0.2 maps to light blue. - -0.1 maps to tan. - 0.0 maps to green. -
 * 1.0 maps to white.
 * <p>
 * The value 0.5 maps to a greenish-white color because 0.5 is halfway between 0.0 (mapped to green)
 * and 1.0 (mapped to white).
 * <p>
 * The value -0.6 maps to a medium blue color because -0.6 is halfway between -1.0 (mapped to dark
 * blue) and -0.2 (mapped to light blue).
 * <p>
 * The color gradient requires a minimum of two gradient points.
 * <p>
 * This class contains two pre-made gradients: a grayscale gradient and a color gradient suitable
 * for terrain. To use these pre-made gradients, call the BuildGrayscaleGradient() or
 * BuildTerrainGradient() methods, respectively.
 *
 * note: The color value passed to AddGradientPoint() has an alpha channel. This alpha channel
 * specifies how a pixel in the background image (if specified) is blended with the calculated
 * color. If the alpha value is high, this class weighs the blend towards the calculated color, and
 * if the alpha value is low, this class weighs the blend towards the color from the corresponding
 * pixel in the background image.
 *
 * <b>Specify the light source parameters</b>
 *
 * This class contains a parallel light source that lights the image. It interprets the noise map as
 * a bump map.
 *
 * To enable or disable lighting, pass a Boolean value to the EnableLight() method.
 *
 * To set the position of the light source in the "sky", call the SetLightAzimuth() and
 * SetLightElev() methods.
 *
 * To set the color of the light source, call the SetLightColor() method.
 *
 * To set the intensity of the light source, call the SetLightIntensity() method. A good intensity
 * value is 2.0, although that value tends to "wash out" very light colors from the image.
 *
 * To set the contrast amount between areas in light and areas in shadow, call the
 * SetLightContrast() method. Determining the correct contrast amount requires some trial and error,
 * but if your application interprets the noise map as a height map that has its elevation values
 * measured in meters and has a horizontal resolution of @a h meters, a good contrast amount to use
 * is ( 1.0 / @a h ).
 *
 * <b>Specify the background image</b>
 *
 * To specify a background image, pass an Image object to the SetBackgroundImage() method.
 *
 * This class determines the color of a pixel in the destination image by blending the calculated
 * color with the color of the corresponding pixel from the background image.
 *
 * The blend amount is determined by the alpha of the calculated color. If the alpha value is high,
 * this class weighs the blend towards the calculated color, and if the alpha value is low, this
 * class weighs the blend towards the color from the corresponding pixel in the background image.
 *
 * <b>Rendering the image</b>
 *
 * To render the image, perform the following steps: - Pass a NoiseMap object to the
 * SetSourceNoiseMap() method. - Pass an Image object to the SetDestImage() method. - Pass an Image
 * object to the SetBackgroundImage() method (optional) - Call the Render() method.
 */
...
public class RendererImage {
    ...
    // To enable the coldness of the poles or not
    private boolean isPoleTemperatureEnabled;
    // The temperature of the poles, larger means colder, and could go downwards
    private double poleTemperature;
    ...
    /**
     * Used by the CalcLightIntensity() method to recalculate the light values only if the light
     * parameters change.
     * <p>
     * When the light parameters change, this value is set to True. When the CalcLightIntensity()
     * method is called, this value is set to false.
     */
    ...
    public RendererImage() {
        ...
        isWrapEnabled = false;
        isPoleTemperatureEnabled = false;
        lightAzimuth = 45.0;
        lightBrightness = 1.0;
        poleTemperature = 1;
     ...
     * <p>
     * This object uses a color gradient to calculate the color for each pixel in the destination
     * image according to the value from the corresponding position in the noise map.
     * <p>
     * The gradient requires a minimum of two gradient points.
     * <p>
     * The specified color value passed to this method has an alpha channel. This alpha channel
     * specifies how a pixel in the background image (if specified) is blended with the calculated
     * color. If the alpha value is high, this object weighs the blend towards the calculated color,
     * and if the alpha value is low, this object weighs the blend towards the color from the
     ...
     * <p>
     * This color gradient contains the following gradient points: - -1.0 maps to black - 1.0 maps
     * to white
     * <p>
     ...
     * <p>
     * This gradient color at position 0.0 is the "sea level". Above that value, the gradient
     * contains greens, browns, and whites. Below that value, the gradient contains various shades
     * of blue.
     * <p>
     ...
     * <p>
     * Before calling the Render() method, the application must specify a new color gradient with at
     ...
     * <p>
     * If the light source is enabled, this object will interpret the noise map as a bump map.
     *
     ...
     * <p>
     * This object requires five points (the initial point and its four neighbors) to calculate
     * light shading. If wrapping is enabled, and the initial point is on the edge of the noise map,
     * the appropriate neighbors that lie outside of the noise map will "wrap" to the opposite
     * side(s) of the noise map. Otherwise, the appropriate neighbors are cropped to the edge of the
     * noise map.
     * <p>
     * Enabling wrapping is useful when creating spherical renderings and tileable textures.
     *
    ...
    /**
     * Returns the azimuth of the light source, in degrees. The azimuth is the location of the light
     * source around the horizon: - 0.0 degrees is east. - 90.0 degrees is north. - 180.0 degrees is
     * west. - 270.0 degrees is south.
     *
     ...
     * <p>
     * The contrast specifies how sharp the boundary is between the light-facing areas and the
     * shadowed areas.
     * <p>
     * The contrast determines the difference between areas in light and areas in shadow.
     * Determining the correct contrast amount requires some trial and error, but if your
     * application interprets the noise map as a height map that has a spatial resolution of @a h
     * meters and an elevation resolution of 1 meter, a good contrast amount to use is ( 1.0 / @a h
     * ).
     *
     ...
     * <p>
     * The elevation is the angle above the horizon: - 0 degrees is on the horizon. - 90 degrees is
     * straight up.
     *
    ...
    /**
     * Renders the destination image using the contents of the source noise map and an optional
     * background image.
     * <p>
     * The background image and the destination image can safely refer to the same image, although
     * in this case, the destination image is irretrievably blended into the background image.
     *
     * @throws IllegalArgumentException See the preconditions. SetSourceNoiseMap() has been
     * previously called. SetDestImage() has been previously called. There are at least two gradient
     * points in the color gradient. No two gradient points have the same position. If a background
     * image was specified, it has the exact same size as the source height map. The original
     * contents of the destination image is destroyed.
     */
    ...
    public void render() {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                //Modify based off latitude
                double latitudePercentage = Math.abs((double) (equator - y)) / ((double) equator);
                ...
                if (!isPoleTemperatureEnabled) {
                    latitudePercentage = 0;
                }

                float mapHeight = pSource.get();

                float colorValue = (float) (mapHeight + latitudePercentage);
                ...
                Color destColor = gradient.getColor(colorValue);

                //Or we can get a higher level
                //latitudePercentage = 1-latitudePercentage;
                //destColor = destColor.multiply(new Color((int) (latitudePercentage * 255), (int) (latitudePercentage * 255), (int) (latitudePercentage * 255), 255));
     ...
     * <p>
     * If a background image has been specified, the Render() method blends the pixels from the
     * background image onto the corresponding pixels in the destination image. The blending weights
     * are determined by the alpha channel in the pixels in the destination image.
     * <p>
     ...
     * <p>
     * The destination image will contain the rendered image after a successful call to the Render()
     * method.
     * <p>
     * The destination image must exist throughout the lifetime of this object unless another image
     * replaces that image
     ...
     * <p>
     * The azimuth is the location of the light source around the horizon: - 0.0 degrees is east. -
     * 90.0 degrees is north. - 180.0 degrees is west. - 270.0 degrees is south.
     * <p>
     * Make sure the light source is enabled via a call to the EnableLight() method before calling
     * the Render() method.
     *
     ...
     * @throws IllegalArgumentException See the preconditions. The
     * specified light contrast is positive.
     */
    ...
    public void enablePoleTemperature() {
        isPoleTemperatureEnabled = true;
    }

    public void setPoleTemperature(double poleTemperature) {
